// Batch import last 10 posts from @gzsmsk via Cloud Function importTelegramByUrl
(async () => {
  try {
    ...
    const idMatches = Array.from(html.matchAll(/https:\/\/t\.me\/gzsmsk\/(\d+)/g)).map(m => m[1]);
    ...
    for (const id of last10) {
      const postUrl = `https://t.me/gzsmsk/${id}`;
